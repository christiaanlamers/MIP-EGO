...
#TODO_CHRIS, probably best to make new class sms-mipego, that inherits from mipego
class mipego(object):
    """
    Generic Bayesian optimization algorithm
    """
    #TODO_CHRIS two surrogate models are needed
    def __init__(self, search_space, obj_func, surrogate, ftarget=None,
                 ...
                 available_gpus=[],bi=True,save_name='test_data'):
        """
        parameter
        ---------
    ...
    def select_candidate(self):
        ...
        if self.bi:
            ...
            for x in self.data:
                x.fitness = x.loss
        #TODO_CHRIS after evaluate run S-metric on all solutions to determine fitness
        #self.data = s_metric(self.data)
        #S-metric should have allready taken place
    ...
    def _initialize(self):
        ...
        #CHRIS after evaluate run S-metric on all solutions to determine fitness in case of bi-objective
    ...
    def _acquisition(self, plugin=None, dx=False, surrogate=None):
        ...
        #TODO_CHRIS here two surrogate functions are needed
        if (surrogate is None):
        ...
        if self.n_point > 1:  # multi-point method
            ...
            #TODO_CHRIS incorporate S-metric here, so Hypervolume improvement must be added to Infillcriteria
            if self.infill == 'EI':
